...
export default function FlowBuilder({
  ...
  useEffect(() => {
    ...
    if (isInitialLoad) {
      ...
      // update the state through setState to preserve history - but only if not currently undoing/redoing
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Check for Ctrl+Y or Cmd+Y or Ctrl+Shift+Z or Cmd+Shift+Z
      else if (isMeta && (event.key === 'y' || (event.shiftKey && event.key === 'Z'))) {
        event.preventDefault();
        handleRedo();
      }
    };
  ...
  const onConnect = useCallback((params: Connection) => {
    if (params.source && params.target) {
      const newConnectionId = `temp-${params.source}-${params.target}-${Date.now()}`;
